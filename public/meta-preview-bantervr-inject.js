// BanterVR UI inject script — self-contained, no story files required.
...
//           src="https://YOUR_DEV_IP:33380/meta-preview-bantervr-inject.js"></script>
...
(function () {
  ...
  window.addEventListener('bs-loaded', function () {
    ...
    async function reconstructScene(sceneData) {
      console.log('[banter-inject] reconstructing', sceneData.objects?.length, 'objects');
      destroyPrev();
      ...
      for (const objData of sceneData.objects || []) {
        ...
        for (const compData of objData.components || []) {
          ...
          console.log('[banter-inject] panel created, oid:', panel.oid);
          ...
          console.log('[banter-inject] panel built');
        }
      }
      console.log('[banter-inject] done');
    }

    function connect() {
      const ws = new WebSocket(wsUrl);
      ws.addEventListener('open', () => {
        console.log('[banter-inject] connected');
        ws.send(JSON.stringify({ type: 'register', role: 'banter-inject' }));
      });
      ws.addEventListener('message', (e) => {
        let msg; try { msg = JSON.parse(e.data); } catch (_) { return; }
        if (msg.type === 'story-rendered' && msg.banterUIData) {
          clearTimeout(ws._debounce);
          ws._debounce = setTimeout(() => {
            reconstructScene(msg.banterUIData).catch(err => console.error('[banter-inject] error:', err));
          }, 1000);
        }
      });
